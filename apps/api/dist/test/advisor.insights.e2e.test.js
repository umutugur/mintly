...
describe('GET /advisor/insights', () => {
    ...
    async function registerUser(email, name = 'Advisor User') {
    ...
    function createCloudflareProviderResponse(summary) {
    ...
    it('returns advisor insight payload for month query and supports regenerate', async () => {
        ...
        const fetchMock = vi.fn(async () => createCloudflareProviderResponse('Nakit akisiniz pozitif ve birikim kapasiteniz korunuyor.'));
    ...
    it('uses cache for non-regenerate requests and bypasses cache on regenerate', async () => {
        ...
        await seedAdvisorData(session.user.id);
        const fetchMock = vi.fn(async () => createCloudflareProviderResponse('Cache test summary.'));
        vi.stubGlobal('fetch', fetchMock);
