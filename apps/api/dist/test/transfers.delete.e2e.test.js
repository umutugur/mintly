...
describe('Transfer delete endpoint', () => {
    ...
    async function registerUser(email, name = 'Transfer User') {
    ...
    it('deletes both sides of a transfer by group id', async () => {
        ...
        const deleteTransfer = await request(app.server)
            .delete(`/transfers/${transfer.body.groupId}`)
            .set(authHeader);
        ...
        expect(deletedTransfers.body.transactions.every((transaction) => transaction.transferGroupId === transfer.body.groupId)).toBe(true);
    });
