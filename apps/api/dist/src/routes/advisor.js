import { advisorActionBudgetInputSchema, advisorActionBudgetResponseSchema, advisorActionRecurringInputSchema, advisorActionRecurringResponseSchema, advisorActionTransferInputSchema, advisorActionTransferResponseSchema, advisorInsightsQuerySchema, recurringRuleSchema, } from '@finsight/shared';
...
const advisorInsightsRateLimitByUser = new Map();
const advisorInsightsRegenerateCooldownByUser = new Map();
function enforceAdvisorInsightsRateLimit(userId) {
    const now = Date.now();
...
function enforceAdvisorRegenerateCooldown(userId, regenerate) {
    if (!regenerate) {
        return;
    }
    const now = Date.now();
...
export function registerAdvisorRoutes(app) {
    app.get('/advisor/insights', { preHandler: authenticate }, async (request) => {
        ...
        const query = parseQuery(advisorInsightsQuerySchema, request.query);
    ...
    app.get('/advisor/provider-health', { preHandler: authenticate }, async (request) => {
        ...
        const modelConfigured = Boolean(config.advisorProvider === 'cloudflare' &&
    ...
    app.post('/advisor/actions/budget', { preHandler: authenticate }, async (request) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const input = parseBody(advisorActionBudgetInputSchema, request.body);
        let createdCount = 0;
        let updatedCount = 0;
        const budgets = [];
    ...
    app.post('/advisor/actions/recurring', { preHandler: authenticate }, async (request, reply) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const input = parseBody(advisorActionRecurringInputSchema, request.body);
    ...
    app.post('/advisor/actions/transfer', { preHandler: authenticate }, async (request, reply) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const input = parseBody(advisorActionTransferInputSchema, request.body);
