import { logoutResponseSchema, recurringCreateInputSchema, recurringListQuerySchema, recurringListResponseSchema, recurringRuleSchema, recurringRunDueResponseSchema, recurringUpdateInputSchema, } from '@finsight/shared';
import { authenticate } from '../auth/middleware.js';
import { getConfig } from '../config.js';
import { ApiError } from '../errors.js';
import { createNormalTransaction, createTransferPair, resolveActiveAccount, resolveActiveCategory, validateTransactionType, } from '../lib/ledger.js';
...
function ensureScheduleFields(cadence, dayOfWeek, dayOfMonth) {
...
function scheduleAtDay(year, month, day, anchor) {
...
function calculateInitialNextRun(cadence, dayOfWeek, dayOfMonth, startAt) {
    ...
    if (candidate.getTime() < startAt.getTime()) {
        candidate = scheduleAtDay(startAt.getUTCFullYear(), startAt.getUTCMonth() + 1, targetDom, startAt);
    }
    return candidate;
}
function advanceNextRun(current, cadence, dayOfWeek, dayOfMonth) {
    ensureScheduleFields(cadence, dayOfWeek, dayOfMonth);
    if (cadence === 'weekly') {
...
function calculateNextFromNow(rule) {
    const now = new Date();
    let candidate = calculateInitialNextRun(rule.cadence, rule.dayOfWeek ?? null, rule.dayOfMonth ?? null, now);
    // Keep advancing until next slot is strictly in the future.
    while (candidate.getTime() <= now.getTime()) {
        candidate = advanceNextRun(candidate, rule.cadence, rule.dayOfWeek ?? null, rule.dayOfMonth ?? null);
    }
    return candidate;
}
async function executeRecurringRun(rule, scheduledFor) {
    if (rule.kind === 'normal') {
...
export function registerRecurringRoutes(app) {
    app.get('/recurring', { preHandler: authenticate }, async (request) => {
        ...
        const query = parseQuery(recurringListQuerySchema, request.query);
        const filter = {
            userId,
    ...
    app.post('/recurring', { preHandler: authenticate }, async (request, reply) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const input = parseBody(recurringCreateInputSchema, request.body);
        ...
        const nextRunAt = calculateInitialNextRun(input.cadence, input.dayOfWeek ?? null, input.dayOfMonth ?? null, startAt);
        const recurringRule = await RecurringRuleModel.create({
            ...
            fromAccountId: input.kind === 'transfer' ? parseObjectId(input.fromAccountId, 'fromAccountId') : null,
    ...
    app.patch('/recurring/:id', { preHandler: authenticate }, async (request) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const recurringId = parseObjectId(request.params.id ?? '', 'id');
        const input = parseBody(recurringUpdateInputSchema, request.body);
        ...
        const scheduleChanged = input.cadence !== undefined || input.dayOfWeek !== undefined || input.dayOfMonth !== undefined;
    ...
    app.post('/recurring/run-due', {
        ...
        for (const rule of dueRules) {
            ...
            while (!shouldPause && cursor.getTime() <= now.getTime()) {
                ...
                try {
                    ...
                    if (error.code === 11000) {
                        ...
                        cursor = advanceNextRun(cursor, rule.cadence, rule.dayOfWeek ?? null, rule.dayOfMonth ?? null);
                        continue;
                    }
                    throw error;
                }
                try {
                    ...
                    await RecurringRunLogModel.updateOne({ _id: logId }, { $set: { generatedTransactionIds: generatedIds } });
                    processedRuns += 1;
            ...
            const hasLastRunChanged = (lastRunAt?.getTime() ?? 0) !== (rule.lastRunAt ? rule.lastRunAt.getTime() : 0);
            if (cursor.getTime() !== rule.nextRunAt.getTime() ||
