import { accountCreateInputSchema, accountListResponseSchema, accountSchema, accountUpdateInputSchema, logoutResponseSchema, } from '@finsight/shared';
import { z } from 'zod';
...
async function enforceBaseCurrency(userId, inputCurrency) {
...
export function registerAccountRoutes(app) {
    ...
    app.post('/accounts', { preHandler: authenticate }, async (request, reply) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const input = parseBody(accountCreateInputSchema, request.body);
    ...
    app.patch('/accounts/:id', { preHandler: authenticate }, async (request) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const accountId = parseObjectId(request.params.id ?? '', 'id');
        const input = parseBody(accountUpdateInputSchema, request.body);
