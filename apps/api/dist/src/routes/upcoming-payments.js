import { upcomingPaymentCreateInputSchema, upcomingPaymentListQuerySchema, upcomingPaymentListResponseSchema, upcomingPaymentMarkPaidInputSchema, upcomingPaymentMarkPaidResponseSchema, upcomingPaymentSchema, upcomingPaymentUpdateInputSchema, } from '@finsight/shared';
import { authenticate } from '../auth/middleware.js';
import { ApiError } from '../errors.js';
import { createNormalTransaction, resolveActiveAccount, validateCurrency, } from '../lib/ledger.js';
...
async function requireBaseCurrency(userId) {
...
async function resolvePreferredExpenseCategory(userId, paymentType) {
...
export function registerUpcomingPaymentRoutes(app) {
    app.get('/upcoming-payments', { preHandler: authenticate }, async (request) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const query = parseQuery(upcomingPaymentListQuerySchema, request.query);
    ...
    app.post('/upcoming-payments', { preHandler: authenticate }, async (request, reply) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const input = parseBody(upcomingPaymentCreateInputSchema, request.body);
    ...
    app.patch('/upcoming-payments/:id', { preHandler: authenticate }, async (request) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const id = parseObjectId(request.params.id ?? '', 'id');
        const input = parseBody(upcomingPaymentUpdateInputSchema, request.body);
    ...
    app.post('/upcoming-payments/:id/mark-paid', { preHandler: authenticate }, async (request) => {
        ...
        const userId = parseObjectId(user.id, 'userId');
        const id = parseObjectId(request.params.id ?? '', 'id');
        const input = parseBody(upcomingPaymentMarkPaidInputSchema, request.body);
        const upcomingPayment = await UpcomingPaymentModel.findOne({
            _id: id,
            userId,
        });
