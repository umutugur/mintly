import { weeklyReportQuerySchema, weeklyReportResponseSchema, } from '@finsight/shared';
...
export function registerReportRoutes(app) {
    app.get('/reports/weekly', { preHandler: authenticate }, async (request) => {
        ...
        const query = parseQuery(weeklyReportQuerySchema, request.query);
        ...
        let totalIncome = 0;
        let totalExpense = 0;
        const weeklyExpenseByCategoryId = new Map();
        for (const transaction of weeklyTransactions) {
            ...
            weeklyExpenseByCategoryId.set(categoryId, (weeklyExpenseByCategoryId.get(categoryId) ?? 0) + transaction.amount);
        }
        const spentByMonthCategoryId = new Map(monthExpenseRows.map((row) => [String(row._id), row.spentAmount]));
        ...
        if (topExpenseCategoryId) {
            highlights.push(`Top expense category: ${categoryNameById.get(topExpenseCategoryId) ?? 'Unknown'} (${Math.round(topExpenseAmount * 100) / 100})`);
        }
        const riskFlags = [];
        if (expenseIncomeRatio > 0.9) {
        ...
        let nextWeekForecastText = 'Maintain current routine and review your dashboard mid-week for adjustments.';
        if (healthScore >= 80) {
