import { aiAdviceQuerySchema, aiInsightsQuerySchema, aiAdviceResponseSchema, } from '@finsight/shared';
...
const aiInsightsRateLimitByUser = new Map();
...
function parseDateOnlyUtc(value, field) {
...
function resolveInsightsDateRange(query) {
    const today = new Date();
    const defaultTo = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate(), 0, 0, 0, 0));
...
function enforceAiInsightsRateLimit(userId) {
    const now = Date.now();
...
export function registerAiRoutes(app) {
    app.get('/ai/advice', { preHandler: authenticate }, async (request) => {
        ...
        const query = parseQuery(aiAdviceQuerySchema, request.query);
        ...
        for (const transaction of transactions) {
            ...
            expenseByCategoryId.set(categoryId, (expenseByCategoryId.get(categoryId) ?? 0) + transaction.amount);
        }
    ...
    app.get('/ai/insights', { preHandler: authenticate }, async (request) => {
        ...
        const userId = parseObjectId(user.id, 'userId');
        const query = parseQuery(aiInsightsQuerySchema, request.query);
