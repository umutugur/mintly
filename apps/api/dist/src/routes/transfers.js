import { logoutResponseSchema, transferCreateInputSchema, transferCreateResponseSchema, } from '@finsight/shared';
...
export function registerTransferRoutes(app) {
    app.post('/transfers', { preHandler: authenticate }, async (request, reply) => {
        ...
        const input = parseBody(transferCreateInputSchema, request.body);
    ...
    app.delete('/transfers/:transferGroupId', { preHandler: authenticate }, async (request) => {
        const user = requireUser(request);
        const userId = parseObjectId(user.id, 'userId');
        const transferGroupId = parseObjectId(request.params.transferGroupId ?? '', 'transferGroupId');
