...
const advisorInsightsCache = new Map();
...
function resolveAnchorDate(monthEndExclusive) {
...
function mapCloudflareErrorToFallbackReason(error) {
...
function resolveCurrentAmountFromLabel(label, categoryBreakdown, merchantBreakdown) {
    ...
    for (const merchant of merchantBreakdown) {
        ...
        if (normalizedMerchant === normalizedLabel ||
...
export async function generateAdvisorInsight(input, fetchImpl = (requestUrl, init) => fetch(requestUrl, init)) {
    ...
    const preferredRiskProfile = (userDoc?.riskProfile ?? 'medium');
    ...
    const expenseByCategoryCurrentMonth = new Map();
    const currentMonthTotals = { income: 0, expense: 0 };
    const last30Totals = { income: 0, expense: 0 };
    const trendByMonth = new Map(trendMonths.map((month) => [month, { income: 0, expense: 0 }]));
    ...
    for (const transaction of transactions) {
        ...
        if (inCurrentMonth) {
            if (transaction.type === 'income') {
                ...
                if (categoryId) {
                    expenseByCategoryCurrentMonth.set(categoryId, (expenseByCategoryCurrentMonth.get(categoryId) ?? 0) + transaction.amount);
                }
            }
        }
        if (inLast30Days) {
    ...
    const totalBalance = roundCurrency(balanceRows.reduce((sum, row) => sum + row.balance, 0));
    const overview = {
        last30DaysIncome,
        last30DaysExpense,
        last30DaysNet,
    ...
    if (cloudflareConfigured && config.cloudflareAuthToken && config.cloudflareAccountId) {
        ...
        try {
            const providerResult = await generateCloudflareText({
                ...
                systemPrompt: 'You are Mintly AI. Return strict JSON only. Do not include markdown fences.',
                userPrompt: prompt,
            ...
            if (parsedProviderJson !== undefined) {
                const parsedProvider = providerOutputSchema.safeParse(parsedProviderJson);
...
export function clearAdvisorInsightsCacheForTests() {
