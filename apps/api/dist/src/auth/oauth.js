...
function requiredAudiences(provider) {
    const config = getConfig();
    const audiences = provider === 'google' ? config.googleOauthClientIds : config.appleOauthClientIds;
...
function validateAppleNonce(payload, rawNonce) {
    if (!rawNonce) {
        return;
    }
...
function parseProviderPayload(provider, payload) {
    ...
    const emailVerified = claimIsTruthy(payload.email_verified);
