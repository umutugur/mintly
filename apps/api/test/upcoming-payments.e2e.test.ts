...
describe('/upcoming-payments', () => {
  ...
  async function registerUser(email: string, name = 'Upcoming User') {
  ...
  async function createAccount(token: string, name = 'Main Account', type = 'bank') {
    ...
    expect(response.status).toBe(201);
    return response.body as { id: string; currency: string };
  }

  async function createExpenseCategory(token: string, name = 'Bills') {
    const response = await request(app.server)
      .post('/categories')
      .set('Authorization', `Bearer ${token}`)
      .send({
  ...
  it('creates upcoming payment, supports listing/filtering, and enforces object-level auth', async () => {
    ...
    const attackerPatch = await request(app.server)
      .patch(`/upcoming-payments/${create.body.id as string}`)
      .set('Authorization', `Bearer ${attacker.accessToken}`)
  ...
  it('mark-paid creates expense transaction and is idempotent', async () => {
    ...
    const firstMarkPaid = await request(app.server)
      .post(`/upcoming-payments/${create.body.id as string}/mark-paid`)
      .set('Authorization', `Bearer ${owner.accessToken}`)
